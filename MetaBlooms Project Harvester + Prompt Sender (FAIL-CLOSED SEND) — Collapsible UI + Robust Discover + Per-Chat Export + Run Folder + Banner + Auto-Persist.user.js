...
If not, your response is incomplete. Regenerate until complete.`;
  ...
  const CFG = {
    ...
    storage: {
      discovered: "mb_discovered_links_v5",
      queue: "mb_queue_v5",
      run: "mb_runstate_v5",
      index: "mb_chat_index_v2",
      ledger: "mb_run_ledger_v2",
      controllerUrl: "mb_controller_url_v5",
      recorder: "mb_recorder_v2",
      runFolderName: "mb_run_folder_name_v2",
      promptDryRun: "mb_prompt_dry_run_v1"
    },
  ...
  async function refreshStatus(extra) {
    ...
    const index = await kvGet(CFG.storage.index, { schema: "mb_chat_index_v2", generated_at: null, items: [] });
    const ledger = await kvGet(CFG.storage.ledger, { schema: "mb_run_ledger_v2", run_id: null, events: [] });
  ...
  async function harvestCurrentConversation() {
    ...
    const record = {
      ...
      turns,
      evidence: { rendered_messages_detected: rendered, selector_path: "data-message-author-role", notes: rendered ? [] : ["Timed out waiting for role-marked message nodes; emitting fail-closed artifact."] }
    };
  ...
  function findSendButton() {
    ...
    const sendish = btns.find(b => (b.textContent || "").trim().toLowerCase() === "send");
    return sendish || null;
  }
  ...
  async function sendPromptInCurrentConversation() {
    ...
    // Verify the prompt is actually present before allowing advance
    await sleep(350);
    let present = false;
    try {
      if (composer.kind === "textarea") {
        present = String(composer.el.value || "").includes("SYSTEM OVERRIDE — MEMORY EXTRACTION MODE");
      } else {
        present = String(composer.el.textContent || "").includes("SYSTEM OVERRIDE — MEMORY EXTRACTION MODE");
      }
    } catch (_) {}
  ...
  async function serialTick() {
    ...
    try {
      if ((run.mode || "HARVEST") === "SEND_PROMPT") {
        ...
        // FAIL-CLOSED: pause immediately if not ok (prevents fast-flip / no paste)
        if (!r.ok) {
          await setRunState({ paused: true });
          await refreshStatus(
        ...
        // Slight throttle so you can see it working
        await sleep(600);
      } else {
